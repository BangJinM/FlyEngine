...
public:
    ...
    {
        m_pData = reinterpret_cast<uint8_t *>(MemoryManager::GetInstance()->Allocate(size, alignment));
    }

    Buffer(const Buffer &rhs)
    {
        m_pData = reinterpret_cast<uint8_t *>(MemoryManager::GetInstance()->Allocate(rhs.m_szSize, rhs.m_szAlignment));
    ...
    Buffer &operator=(const Buffer &rhs)
    {
        ...
        {
            if (m_pData)
                MemoryManager::GetInstance()->Free(m_pData, m_szSize, m_szAlignment);
            m_pData = reinterpret_cast<uint8_t *>(MemoryManager::GetInstance()->Allocate(rhs.m_szSize, rhs.m_szAlignment));
            memcpy(m_pData, rhs.m_pData, rhs.m_szSize);
            m_szSize      = rhs.m_szSize;
            m_szAlignment = rhs.m_szAlignment;
