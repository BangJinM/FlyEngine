...
{
public:
    explicit FBuffer(PHYSFS_File *file, std::size_t bufferSize = 2048) : bufferSize(bufferSize), file(file)
    {
...
private:
    ...
    pos_type seekoff(off_type pos, ios_base::seekdir dir, ios_base::openmode mode) override
    {
        switch (dir)
        {
            case std::ios_base::beg: PHYSFS_seek(file, pos); break;
            case std::ios_base::cur:
                // Subtract characters currently in buffer from seek position.
                PHYSFS_seek(file, (PHYSFS_tell(file) + pos) - (egptr() - gptr()));
                break;
            case std::ios_base::end: PHYSFS_seek(file, PHYSFS_fileLength(file) + pos); break;
        }
...
PHYSFS_File *OpenWithMode(const std::filesystem::path &filename, FileMode openMode)
{
    ...
    switch (openMode)
    {
        case FileMode::Write: file = PHYSFS_openWrite(pathStr.c_str()); break;
        case FileMode::Append: file = PHYSFS_openAppend(pathStr.c_str()); break;
        case FileMode::Read: file = PHYSFS_openRead(pathStr.c_str());
    }
...
IFStream::IFStream(const std::filesystem::path &filename) : BaseFStream(OpenWithMode(filename, FileMode::Read)), std::istream(new FBuffer(file)) {}
...
OFStream::OFStream(const std::filesystem::path &filename, FileMode writeMode)
    : BaseFStream(OpenWithMode(filename, writeMode)), std::ostream(new FBuffer(file))
{}
...
FStream::FStream(const std::filesystem::path &filename, FileMode openMode)
    : BaseFStream(OpenWithMode(filename, openMode)), std::iostream(new FBuffer(file))
{}
...
Files::Files(std::string arg)
{
    PHYSFS_init(arg.c_str());
}
...
std::vector<unsigned char> Files::ReadBytes(const std::filesystem::path &path)
{
    ...
    std::copy(std::istream_iterator<unsigned char>(file), std::istream_iterator<unsigned char>(), back_inserter(bytes));
