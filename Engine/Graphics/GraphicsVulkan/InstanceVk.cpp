...
void InstanceVk::Initialize()
{
    CreateInstance();
    SetupDebugMessenger();
}

void InstanceVk::Finalize()
{
...
void InstanceVk::CreateInstance()
{
    ...
    if (err != VK_SUCCESS)
    {
        switch (err)
        {
            case VK_ERROR_INCOMPATIBLE_DRIVER: std::runtime_error("Cannot find a compatible Vulkan installable client driver (ICD).\n\n"); break;
            case VK_ERROR_EXTENSION_NOT_PRESENT: std::runtime_error("Cannot find a specified extension library.\n"); break;
            default: std::runtime_error("vkCreateInstance failed.\n\n"); break;
        }
    }
}
...
void InstanceVk::SetupDebugMessenger()
{
    if (!enableValidationLayers)
        return;
