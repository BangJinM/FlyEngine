...
typedef std::vector<DynamicStateFlagBit> DynamicStateList;
...
{
    // The application does not synchronizes with the vertical sync. If application renders faster than the display refreshes, frames are wasted and
    // tearing may be observed. FPS is uncapped. Maximum power consumption. If unsupported, "ON" value will be used instead. Minimum latency.
    OFF,
    // The application is always synchronized with the vertical sync. Tearing does not happen. FPS is capped to the display's refresh rate. For fast
    // applications, battery life is improved. Always supported.
    ON,
    ...
    // there is no tearing. Compared to ON, the FPS will be improved for "slower" applications. If unsupported, "ON" value will be used instead.
    // Recommended for most applications. Default if supported.
    RELAXED,
    // The presentation engine will always use the latest fully rendered image. Compared to OFF, no tearing will be observed. Compared to ON, battery
    // power will be worse, especially for faster applications. If unsupported,  "OFF" will be attempted next.
    MAILBOX,
    // The application is capped to using half the vertical sync time. FPS artificially capped to Half the display speed (usually 30fps) to maintain
    // battery. Best possible battery savings. Worst possible performance. Recommended for specific applications where battery saving is critical.
    HALF,
};
...
struct Rect
{
    ...
    {
        if (x == rs.x && y == rs.y && width == rs.width && height == rs.height)
        {
            return true;
        }
        else
    ...
    bool operator!=(const Rect &rs) { return !(*this == rs); }
};
...
typedef std::vector<BufferTextureCopy> BufferTextureCopyList;
typedef std::vector<const uint8_t *>   BufferDataList;
...
struct Viewport
{
    ...
    {
        return (left == rs.left && top == rs.top && width == rs.width && height == rs.height && math::IsEqualF(minDepth, rs.minDepth) &&
                math::IsEqualF(maxDepth, maxDepth));
    }

    bool operator!=(const Viewport &rs) { return !(*this == rs); }
};
...
struct Color
{
    ...
    {
        return (math::IsEqualF(x, rhs.x) && math::IsEqualF(y, rhs.y) && math::IsEqualF(z, rhs.z) && math::IsEqualF(w, rhs.w));
    }
};
#pragma pack(pop)
...
typedef std::vector<ShaderMacro> ShaderMacroList;
...
typedef std::vector<DescriptorSetLayoutBinding> DescriptorSetLayoutBindingList;
...
typedef std::vector<DescriptorSetLayout *> DescriptorSetLayoutList;
...
typedef std::vector<BlendTarget> BlendTargetList;
...
typedef std::vector<CommandBuffer *> CommandBufferList;
...
extern const uint DESCRIPTOR_BUFFER_TYPE;
extern const uint DESCRIPTOR_SAMPLER_TYPE;
extern const uint DESCRIPTOR_DYNAMIC_TYPE;
