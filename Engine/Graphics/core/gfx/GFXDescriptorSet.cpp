...
DescriptorSet::DescriptorSet(Device *device) : GFXObject(ObjectType::DESCRIPTOR_SET), _device(device) {}

DescriptorSet::~DescriptorSet() {}

void DescriptorSet::bindBuffer(uint binding, Buffer *buffer, uint index)
{
    const std::vector<uint>              &bindingIndices = _layout->getBindingIndices();
    const DescriptorSetLayoutBindingList &bindings       = _layout->getBindings();
    if (binding >= bindingIndices.size() || bindingIndices[binding] >= bindings.size())
        return;
...
{
    const std::vector<uint> &descriptorIndices = _layout->getDescriptorIndices();
    if (binding >= descriptorIndices.size())
        return nullptr;
    const uint descriptorIndex = descriptorIndices[binding] + index;
    if (descriptorIndex >= _buffers.size())
        return nullptr;
...
Texture *DescriptorSet::getTexture(uint binding, uint index) const
{
    ...
    if (binding >= descriptorIndices.size())
        return nullptr;
    const uint descriptorIndex = descriptorIndices[binding] + index;
    if (descriptorIndex >= _textures.size())
        return nullptr;
...
Sampler *DescriptorSet::getSampler(uint binding, uint index) const
{
    ...
    if (binding >= descriptorIndices.size())
        return nullptr;
    const uint descriptorIndex = descriptorIndices[binding] + index;
    if (descriptorIndex >= _samplers.size())
        return nullptr;
