...
bool CCVKDevice::initialize(const DeviceInfo &info)
{
    ...
    if (context->minorVersion() < 1 && !context->checkExtension(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME))
    {
    ...
    if (checkExtension(VK_KHR_DEDICATED_ALLOCATION_EXTENSION_NAME) && checkExtension(VK_KHR_GET_MEMORY_REQUIREMENTS_2_EXTENSION_NAME))
    {
    ...
    _gpuDevice->swapchain = _gpuSwapchain = new (CCVKGPUSwapchain);
    checkSwapchainStatus();

    ///////////////////// Print Debug Info /////////////////////

    std::string instanceLayers, instanceExtensions, deviceLayers, deviceExtensions;
    for (const char *layer : ((CCVKContext *)_context)->getLayers())
    {
        instanceLayers += layer + std::string(" ");
    }
    for (const char *extension : ((CCVKContext *)_context)->getExtensions())
    {
        instanceExtensions += extension + std::string(" ");
    }
    for (const char *layer : _layers)
    {
        deviceLayers += layer + std::string(" ");
    }
    for (const char *extension : _extensions)
    {
        deviceExtensions += extension + std::string(" ");
    }
    ...
    _version            = StringUtil::Format("%d.%d.%d", VK_VERSION_MAJOR(apiVersion), VK_VERSION_MINOR(apiVersion), VK_VERSION_PATCH(apiVersion));
...
void CCVKDevice::destroy()
{
    ...
    if (_gpuDevice)
    {
        ...
        for (CCVKGPUDevice::CommandBufferPools::iterator it = _gpuDevice->commandBufferPools.begin(); it != _gpuDevice->commandBufferPools.end();
             ++it)
        {
...
void CCVKDevice::acquire()
{
    ...
    VK_CHECK(vkAcquireNextImageKHR(_gpuDevice->vkDevice, _gpuSwapchain->vkSwapchain, ~0ull, acquireSemaphore, VK_NULL_HANDLE,
                                   &_gpuSwapchain->curImageIndex));
...
void CCVKDevice::present()
{
    ...
    if (queue->gpuQueue()->nextWaitSemaphore)
    {  // don't present if not acquired
        ...
        if (fenceCount)
        {
            VK_CHECK(vkWaitForFences(_gpuDevice->vkDevice, fenceCount, gpuFencePool()->data(), VK_TRUE, DEFAULT_TIMEOUT));
        }
...
bool CCVKDevice::checkSwapchainStatus()
{
    ...
    auto flag = vkGetPhysicalDeviceSurfaceCapabilitiesKHR(context->physicalDevice, context->vkSurface, &surfaceCapabilities);
    VK_CHECK(flag);
    ...
    if (context->swapchainCreateInfo.preTransform == preTransform && context->swapchainCreateInfo.imageExtent.width == newWidth &&
    ...
    std::vector<VkImageMemoryBarrier> barriers(imageCount * 2, {VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER});
    for (uint i = 0u; i < imageCount; i++)
    {
        ...
        barriers[imageCount + i].subresourceRange.aspectMask =
    ...
    for (FramebufferListMapIter it = _gpuSwapchain->vkSwapchainFramebufferListMap.begin(); it != _gpuSwapchain->vkSwapchainFramebufferListMap.end();
         it++)
    {
